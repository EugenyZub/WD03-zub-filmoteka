...
function film_update($link, $title, $genre, $year, $id, $description, $photo){
	...
	if (isset($_FILES['photo']['name']) && $_FILES['photo']['tmp_name'] != "" ) {
		...
		$kaboom = explode(".", $fileName); //разбиваем имя файла на 2 части: до точки и после 
		$fileExt = end($kaboom);

		list($width, $height) = getimagesize($fileTmpLoc);//получение размера изображение
		if ($width < 10 || $height < 10) {
			$errors[] = 'That image has no dimensions';
		}

		$db_file_name = rand(10000, 999999) . "." . $fileExt;//переименовываем файл с помощью случайного числа
		//проверки размера, нрасширения загружаемого файла
		if ($fileSize > 1048576) {
			$errors[] = 'Your image file was larger then 1mb';
		} else if(!preg_match("/\.(gif|jpg|png|jpeg)$/i", $fileName) ) {
			$errors[] = 'Your image file was not jpg, jpeg, gif or png type';
		} else if($fileErrorMsg == 1) {
			$errors[] = 'An unkown error occurred';
		}
